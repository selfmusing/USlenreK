...
// for do_execveat_common on >= 3.19, struct filename **filename
...
#else
// for do_execve_common on < 3.19, char **filename
int ksu_legacy_execve_sucompat(int *fd, const char **filename_ptr,
				 void *__never_use_argv, void *__never_use_envp,
				 int *__never_use_flags)
{
	if (!is_su_allowed((const void *)filename_ptr))
